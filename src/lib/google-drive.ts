...
async function listDriveFolderItems(folderId: string): Promise<DriveFolderItem[]> {
  const apiKey = process.env.GOOGLE_DRIVE_API_KEY;
...
/**
 * Recursively lists individual PDFs inside a public Drive folder. Folder
 * shortcuts are followed, PDF shortcuts are returned as the target PDF, and
 * visited folder ids prevent shortcut cycles.
 */
export async function listDriveFolderTreePdfs(
  folderId: string,
  ...
  async function walk(currentFolderId: string, folderPath: string[]) {
    if (visitedFolders.has(currentFolderId)) return;
    visitedFolders.add(currentFolderId);

    const items = await listDriveFolderItems(currentFolderId);
    for (const item of items) {
      ...
      if (targetMimeType === "application/vnd.google-apps.folder") {
        await walk(targetId, [...folderPath, item.name]);
        continue;
      }
      if (targetMimeType !== "application/pdf" || seenFiles.has(targetId)) {
        continue;
      }
      seenFiles.add(targetId);
      files.push({
        id: targetId,
        name: item.name,
        webViewLink:
