 ...
 * doesn't apply yet. */
export async function getResolvedThemeForNote(subjectId: string, subjectNotesId: string): Promise<ThemeValues> {
  const [globalTheme, subjectTheme, noteTheme] = await Promise.all([
    ...
    prisma.noteTheme.findUnique({ where: { subjectNotesId } }),
  ]);
