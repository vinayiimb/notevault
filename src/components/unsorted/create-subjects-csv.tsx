...
import { createSubjectsFromCsvAction, type NewSubjectCsvRowResult } from "@/lib/actions";
...
export function CreateSubjectsCsv() {
  const inputRef = useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState<NewSubjectCsvRowResult[] | null>(null);
  ...
  async function run(file: File) {
    ...
    try {
      ...
      const { results } = await createSubjectsFromCsvAction(formData);
      setResults(results);
    } catch (err) {
  ...
  const issues = results?.filter((r) => r.status !== "created") ?? [];
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      ...
      {running && <p className="mt-3 text-sm text-muted">Creating…</p>}
      {error && <p className="mt-3 text-sm text-red-500">{error}</p>}

      {results && (
        <div className="mt-4">
          <p className="text-sm font-medium">
            {createdCount} of {results.length} row{results.length === 1 ? "" : "s"} created
          </p>
          {issues.length > 0 && (
