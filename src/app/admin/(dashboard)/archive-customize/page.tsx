...
import { getArchiveCourseOverview, getAllMergedSubjects } from "@/lib/archive-customize-data";
import { resetCatalogSubjectOverrideAction } from "@/lib/actions";

export const dynamic = "force-dynamic";

export default async function ArchiveCustomizePage() {
  let courses;
  let allMerged: Awaited<ReturnType<typeof getAllMergedSubjects>> = [];
  let loadError: string | null = null;
  try {
    const [c, m] = await Promise.all([getArchiveCourseOverview(), getAllMergedSubjects()]);
    courses = c;
    allMerged = m;
  } catch (err) {
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-border bg-surface-muted text-xs uppercase tracking-wide text-muted">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">Merged subjects</th>
              ...
              <tbody>
                {courses!.map((c) => (
                  <tr key={c.course} className="border-b border-border/60 last:border-0 hover:bg-surface-muted">
          ...
          {allMerged.length > 0 && (
            <section className="mt-12 space-y-4">
              <div className="border-b border-border pb-2">
                <h2 className="font-display text-xl font-bold text-foreground">Global List of Merged Subjects</h2>
                <p className="text-sm text-muted">All subjects you have merged across all programmes.</p>
              </div>
              <div className="overflow-hidden rounded-2xl border border-border bg-surface p-5">
                <ul className="space-y-4">
                  {allMerged.map((s) => (
                    <li key={s.overrideId!} className="flex flex-col gap-2 border-b border-border/50 pb-4 last:border-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="rounded bg-surface-muted px-1.5 py-0.5 text-[10px] font-bold text-muted uppercase">
                            {s.course}
                          </span>
                          <span className="font-semibold text-accent">{s.displayName}</span>
                        </div>
                        <p className="mt-1.5 text-xs text-muted">
                          Originally matched {s.rawVariants.length} spellings: {s.rawVariants.join(", ")}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/admin/archive-customize/${s.courseSlug}`}
                          className="text-[11px] font-semibold text-muted hover:text-foreground hover:underline"
                        >
                          View course
                        </Link>
                        <form action={resetCatalogSubjectOverrideAction}>
                          <input type="hidden" name="id" value={s.overrideId!} />
                          <input type="hidden" name="courseSlug" value={s.courseSlug} />
