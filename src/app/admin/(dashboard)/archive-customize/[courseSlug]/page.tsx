...
export default async function ArchiveCustomizeCoursePage({
  ...
  return (
    <div className="space-y-8 p-6 sm:p-8">
      ...
      {/* Merged Subjects Summary */}
      {detail.allSubjects.filter(s => s.hasOverride).length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-bold text-foreground">Merged / Renamed Subjects</h2>
          <div className="overflow-hidden rounded-2xl border border-border bg-surface p-5">
            <ul className="space-y-3">
              {detail.allSubjects.filter(s => s.hasOverride).map(s => (
                <li key={s.subjectKey} className="flex flex-wrap items-start justify-between gap-2 border-b border-border/50 pb-3 last:border-0 last:pb-0">
                  <div>
